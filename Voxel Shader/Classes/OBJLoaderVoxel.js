...
// OBJDocVoxel object
// Constructor
var OBJDocVoxel = function (fileName) {
...
OBJDocVoxel.prototype.parse = function (fileString, scale, reverse) {
...
OBJDocVoxel.prototype.parseMtllib = function (sp, fileName) {
    // Get directory path
...
OBJDocVoxel.prototype.parseObjectName = function (sp) {
    var name = sp.getWord();
    return (new OBJObject(name));
};

OBJDocVoxel.prototype.parseVertex = function (sp, scale) {
    var xfloat = sp.getFloat();
    var yfloat = sp.getFloat();
    var zfloat = sp.getFloat();
    var x = Number(xfloat.toFixed(1)) * scale;
    var y = Number(yfloat.toFixed(1)) * scale;
    var z = Number(zfloat.toFixed(1)) * scale;
    if((x*10)%2 == 1)
    {
        x = x+0.1;
    }
...
OBJDocVoxel.prototype.parseNormal = function (sp) {
    //var x = sp.getFloat();
    //var y = sp.getFloat();
    //var z = sp.getFloat();
    var xfloat = sp.getFloat();
    var yfloat = sp.getFloat();
    var zfloat = sp.getFloat();
    var x = Number(xfloat.toFixed(1));
    var y = Number(yfloat.toFixed(1));
    var z = Number(zfloat.toFixed(1));
    return (new VecMath.SFVec3f(x, y, z));
};

OBJDocVoxel.prototype.parseTexCoord = function (sp) {
    var x = sp.getFloat();
    var y = sp.getFloat();
    return (new VecMath.SFVec2f(x, y));
};

OBJDocVoxel.prototype.parseUsemtl = function (sp) {
    return sp.getWord();
};

OBJDocVoxel.prototype.parseFace = function (sp, materialName, positions, reverse) {
    var face = new Face(materialName);
    var vertexindices = new Array(0);
    var textureindices = new Array(0);
    var normalindices = new Array(0);
    // get indices
    for (; ;) {
        var word = sp.getWord();
        if (word == null) break;
    ...
    var xv0 =  positions[vertexindices[0]].x;
    var xv1 =  positions[vertexindices[1]].x;
    var xv2 =  positions[vertexindices[2]].x;
    var yv0 =  positions[vertexindices[0]].y;
    var yv1 =  positions[vertexindices[1]].y;
    var yv2 =  positions[vertexindices[2]].y;
    var zv0 =  positions[vertexindices[0]].z;
    var zv1 =  positions[vertexindices[1]].z;
    var zv2 =  positions[vertexindices[2]].z;
    
    var xmean = (xv0 + xv1 +xv2)/3;
    var ymean = (yv0 + yv1 +yv2)/3;
    var zmean = (zv0 + zv1 +zv2)/3;
    var mindif;
    var xmindif = Infinity;
    var xclosetomin;

    if(difference(xmean, xv0)< xmindif)
    {
        xmindif = difference(xmean,xv0);
        xclosetomin = xv0;
    }
    else if(difference(xmean, xv1)< xmindif)
    {
        xmindif = difference(xmean,xv1);
        xclosetomin = xv1;
    }
    else if(difference(xmean, xv2)< xmindif)
    {
        xmindif = difference(xmean,xv2);
        xclosetomin = xv2;
    }
    
    var ymindif = Infinity;
    var yclosetomin;

    if(difference(ymean, yv0)< ymindif)
    {
        ymindif = difference(ymean,yv0);
        yclosetomin = yv0;
    }
    else if(difference(ymean, yv1)< ymindif)
    {
        ymindif = difference(ymean,yv1);
        yclosetomin = yv1;
    }
    else if(difference(ymean, yv2)< ymindif)
    {
        ymindif = difference(ymean,yv2);
        yclosetomin = yv2;
    }
    var zmindif = Infinity;
    var zclosetomin;

    if(difference(zmean, zv0)< zmindif)
    {
        zmindif = difference(zmean,zv0);
        zclosetomin = zv0;
    }
    else if(difference(zmean, zv1)< zmindif)
    {
        zmindif = difference(zmean,zv1);
        zclosetomin = zv1;
    }
    else if(difference(zmean, zv2)< zmindif)
    {
        zmindif = difference(zmean,zv2);
        zclosetomin = zv2;
    }
    mindif = Math.min(xmindif, ymindif, zmindif);
...
// Check Materials
OBJDocVoxel.prototype.isMTLComplete = function () {
...
OBJDocVoxel.prototype.findColor = function (name) {
    for (var i = 0; i < this.mtls.length; i++) {
...
OBJDocVoxel.prototype.findTexture = function (name) {
    for (var i = 0; i < this.mtls.length; i++) {
...
OBJDocVoxel.prototype.getDrawingInfo = function () {
